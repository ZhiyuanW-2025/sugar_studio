...
export function ProjectRepositoryPanel({ projectId, onNotice }: { projectId: string | null; onNotice: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/projects/repository?projectId=${encodeURIComponent(projectId)}`, { cache: "no-store" });
      const payload = await response.json().catch(() => null) as RepositoryResponse | null;
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch("/api/projects/repository", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId, repository: { localRepositoryPath, remoteName, remoteUrl } }),
      });
      const payload = await response.json().catch(() => null) as RepositoryResponse | null;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" aria-busy={busy === "save"} disabled={!!busy || !localRepositoryPath.trim() || !remoteName.trim()} onClick={save} className="h-8 rounded-md bg-[#252522] px-3 text-control font-medium text-white disabled:opacity-40">{busy === "save" ? "验证中…" : repository ? "更新绑定" : "验证并绑定"}</button>
