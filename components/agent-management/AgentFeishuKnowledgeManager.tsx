...
export function AgentFeishuKnowledgeManager({ agent }: { agent: AgentDefinition }) {
  ...
  const load = async () => {
    ...
    const payload = await response.json().catch(() => null) as { scope?: Scope | null; error?: string } | null;
    setLoading(false);
