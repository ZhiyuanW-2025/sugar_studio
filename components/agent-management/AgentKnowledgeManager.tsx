...
export function AgentKnowledgeManager({ agent }: { agent: AgentDefinition }) {
  ...
  const save = async () => { if (!selected.title.trim() || !selected.content.trim() || saving) return; setSaving(true); setError(undefined); const response = await fetch("/api/agents/knowledge", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ agentType: agent.type, ...selected, syncToFeishu: !selected.id }) }); const payload = await response.json().catch(() => null) as { error?: string } | null; setSaving(false); if (!response.ok) { setError(payload?.error || "保存通用知识失败。"); return; } setNotice(selected.id ? "Agent 通用知识已保存" : "Agent 通用知识已保存并同步至飞书"); window.setTimeout(() => setNotice(undefined), 2200); await load(); };
