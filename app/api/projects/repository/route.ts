...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ repository: data ? mapRepositoryRow(data, workspace) : null, localWorkspaceConfigured: Boolean(workspace) }, { headers });
  } catch (error) {
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const status = await getLocalGitProvider(user.id).getRepositoryStatus(candidate);
