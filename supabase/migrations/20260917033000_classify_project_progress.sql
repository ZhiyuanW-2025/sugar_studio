...
create function public.classify_project_activity()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  ...
  new.category := case
    ...
    when new.event_type in ('project_file_uploaded', 'project_file_version_uploaded', 'project_file_deleted', 'project_file_moved_out', 'design_image_approved', 'design_image_approval_revoked', 'client_deliverable_approved') then 'material'
...
create trigger project_activities_classify_before_insert
before insert on public.project_activities
for each row execute function public.classify_project_activity();
